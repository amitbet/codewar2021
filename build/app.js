...
class GameCoordinator {
  ...
  initBotSelection() {
    let botLines = document.getElementsByClassName("load-src-input");
    for (let b of botLines) {
      b.addEventListener(
        'click',
        this.toggleBotSrc.bind(this, b),
      );
    }
  }

  getColorHexStr(colorRGBArray) {
    return colorRGBArray[0].toString(16) + colorRGBArray[1].toString(16) + colorRGBArray[2].toString(16);
  };

  toggleBotSrc(srcInput) {
    var senderInd = srcInput.attributes['src-ind'].value;
    if (this.srcIndices[1] === senderInd) {
      this.srcIndices[1] = -1;
    } else if (this.srcIndices[0] === senderInd) {
      this.srcIndices[0] = -1;
    } else if (this.srcIndices[1] === -1) {
      this.srcIndices[1] = senderInd;
    } else if (this.srcIndices[0] === -1) {
      this.srcIndices[0] = senderInd;
    }
  ...
  markBotSrcLines() {
    ...
    for (i = 0; i < srcCount; i++) {
      ...
      if ('' + i === this.srcIndices[0]) {
        elm.classList.add('src-0');
        elm.classList.remove('src-1');
      } else if ('' + i === this.srcIndices[1]) {
  ...
  loadBotSource(index) {
    ...
    console.log('loading source: ' + srcText);
    if (srcText) {
  ...
  reset() {
    ...
    this.ghosts = [
      this.blinky, this.pinky, this.inky,
      //this.clyde
    ];
